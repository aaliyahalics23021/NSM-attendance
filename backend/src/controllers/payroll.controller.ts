...
export const generatePayroll = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    for (const employee of employees) {
      ...
      // Save to Database
      const payrollItem = await prisma.payroll.upsert({
        where: {
...
export const getPayroll = async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  ...
  try {
    if (role === 'EMPLOYEE') {
      if (!employeeId) return res.status(403).json({ message: 'Employee missing context' });
...
export const getSalaryRules = async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  ...
  try {
    const rules = await prisma.salaryRule.findUnique({
      where: { companyId }
    });
...
export const updateSalaryRules = async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
